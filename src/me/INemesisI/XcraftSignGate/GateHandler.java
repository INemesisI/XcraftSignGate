...
public class GateHandler {
	...
	public void load() {
		...
		for (String id : config.getKeys(false)) {
			...
			boolean closed;
			List<String> temp;
			temp = config.getList(id+".Signs");
			for(String s : temp) {
				signs.add(StringToBlock(s));
			}
			temp = config.getList(id+".Fences");
			for(String s : temp) {
				fences.add(StringToBlock(s));
			}
	...
	public Block getFenceInVertDirection(Block block, int vert) {
		for (int i=vert;i>=0;i--) {
			...
			if (checkdown.getType().equals(Material.FENCE))
					return checkdown;
		}
		for (int i=1;i<=vert;i++) {
			
			Block checkup = block.getRelative(BlockFace.UP, i);
			if (checkup.getType().equals(Material.FENCE))
					return checkup;
		}
		return null;
